...
This file is part of the Quake4Doom source code by Justin Marshall.
...
void idWindow::Draw(int time, float x, float y) {
	...
	// Quake 4 extends idDeviceContext with textStyle / textSpacing aware drawing.
	// The stock Doom 3 API does not expose that overload, so we preserve the parsed
	// state and fall back to classic DrawText here unless the renderer is extended.
	dc->DrawText(text, textScale, textAlign, foreColor, textRect, !(flags & WIN_NOWRAP), -1);

	if (gui_edit.GetBool()) {
		dc->EnableClipping(false);
		dc->DrawText(va("x: %i  y: %i", (int)rect.x(), (int)rect.y()), 0.25f, 0, dc->colorWhite, idRectangle(rect.x(), rect.y() - 15, 100, 20), false);
		dc->DrawText(va("w: %i  h: %i", (int)rect.w(), (int)rect.h()), 0.25f, 0, dc->colorWhite, idRectangle(rect.x() + rect.w(), rect.w() + rect.h() + 5, 100, 20), false);
...
bool idWindow::RunTimeEvents(int time) {
	...
	if (!(flags & WIN_ALWAYSTHINK) && time - lastTimeRun < USERCMD_MSEC) {
		return false;
	}
	...
	if (visible && !noEvents) {
		...
		if (event->evType == SE_KEY) {
			...
			if (event->evValue == K_MOUSE1) {
				...
				if (event->evValue2 && !actionDownRun) {
					actionDownRun = RunScript(ON_ACTIONRIGHT);
				}
			}
			...
			else if (event->evValue == K_JOY3 && event->evValue2) {
				if (GetFocusedChild()) {
					const char* childRet = GetFocusedChild()->HandleEvent(event, updateVisuals);
					if (childRet && *childRet) {
						return childRet;
					...
					if (!actionDownRun) {
						actionDownRun = GetFocusedChild()->RunScript(ON_ACTION);
						AddCommand(GetFocusedChild()->cmd.c_str());
					}
				}
				RunControllerScript(ON_JOYACTION, event, updateVisuals);
			}
			else if (event->evValue == K_JOY4 && event->evValue2) {
				if (!RunControllerScript(ON_JOYBACK, event, updateVisuals)) {
					if (GetFocusedChild()) {
						const char* childRet = GetFocusedChild()->HandleEvent(event, updateVisuals);
						if (childRet && *childRet) {
							return childRet;
			...
			else if (event->evValue == K_JOY1 && event->evValue2) {
				RunControllerScript(ON_JOYLSHOULDER, event, updateVisuals);
			}
			else if (event->evValue == K_JOY2 && event->evValue2) {
				RunControllerScript(ON_JOYRSHOULDER, event, updateVisuals);
			}
			else if (event->evValue == K_JOY5 && event->evValue2) {
				RunControllerScript(ON_JOYBUTTON2, event, updateVisuals);
			}
			else if (event->evValue == K_JOY6 && event->evValue2) {
				RunControllerScript(ON_JOYBUTTON1, event, updateVisuals);
			}
			else if (event->evValue == K_JOY7 && event->evValue2) {
				RunControllerScript(ON_JOYBUTTON3, event, updateVisuals);
			}
			else if (event->evValue == K_JOY9 && event->evValue2) {
				RunControllerScript(ON_JOYLEFT, event, updateVisuals);
			}
			else if (event->evValue == K_JOY10 && event->evValue2) {
				RunControllerScript(ON_JOYRIGHT, event, updateVisuals);
			}
			else if (event->evValue == K_JOY11 && event->evValue2) {
				RunControllerScript(ON_JOYUP, event, updateVisuals);
			}
			else if (event->evValue == K_JOY12 && event->evValue2) {
				RunControllerScript(ON_JOYDOWN, event, updateVisuals);
			}
			else {
				if (GetFocusedChild()) {
					const char* childRet = GetFocusedChild()->HandleEvent(event, updateVisuals);
		...
		else if (event->evType == SE_JOYSTICK_AXIS) {
			if (updateVisuals) {
				*updateVisuals = true;
			}
...
void idWindow::SetupFromState() {
	...
	CalcClientRect(0, 0);
	if (scripts[ON_ACTION] || scripts[ON_ACTIONRIGHT] || scripts[ON_JOYACTION]) {
...
idWindow* idWindow::SetFocus(idWindow* w, bool scripts) {
	...
	if (scripts && lastFocus) {
		lastFocus->RunScript(ON_LOSEFOCUS);
	}
	if (scripts && w) {
		w->RunScript(ON_GAINFOCUS);
	}
...
idWinVar* idWindow::GetWinVarByName(const char* _name, bool fixup, drawWin_t** owner) {
	...
	if (fixup) {
		...
		if (n > 0) {
			...
			idStr varName = key.Right(key.Length() - n - 2);
			drawWin_t* win = GetGui()->GetDesktop()->FindChildByName(winName);
			if (win) {
				if (win->win) {
					return win->win->GetWinVarByName(varName, false, owner);
				}
				else {
					if (owner) {
						*owner = win;
					}
					return win->simp->GetWinVarByName(varName);
				}
			}
		}
	}
	...
	char* underscore = strrchr(const_cast<char*>(_name), '_');
	if (underscore && underscore[1] && underscore[2] == '\0' && !preventRecursion) {
		idStr temp = _name;

		int index = -1;
		const char suffix = (char)tolower(underscore[1]);

		preventRecursion = true;
		idWinVar* base = GetWinVarByName(temp.c_str(), fixup, owner);
		preventRecursion = false;

		if (dynamic_cast<idWinVec4*>(base)) {
			switch (suffix) {
		...
		else if (dynamic_cast<idWinRectangle*>(base)) {
			switch (suffix) {
			case 'x': index = 0; break;
			case 'y': index = 1; break;
		...
		if (index != -1) {
			idWinFloatMember* member = new idWinFloatMember(base, index);
...
drawWin_t* idWindow::FindChildByName(const char* _name) {
	...
	int c = drawWindows.Num();
	int start = startChildSearch;
	if (start >= c) {
		start = 0;
	}

	for (int phase = 0; phase < 2; phase++) {
		int i = (phase == 0) ? start : 0;
		int end = (phase == 0) ? c : start;
		for (; i < end; i++) {
			if (drawWindows[i].win) {
...
void idWindow::ResetTime(int t) {
	...
	for (int i = 0; i < transitions.Num(); i++) {
		...
		if (data->interp.IsDone(gui->GetTime()) && data->data) {
			...
			if (i < transitionSubs.Num()) {
				transitionSubs.RemoveIndex(i);
			}
			i--;
		}
	}
...
void idWindow::WriteSaveGameTransition(idTransitionData& trans, idFile* savefile) {
	...
	idWinVar* targetVar = trans.data;
	int subIndex = -1;
	if (idWinFloatMember* member = dynamic_cast<idWinFloatMember*>(trans.data)) {
		targetVar = member->GetOwner();
		subIndex = member->GetComponent();
	}

	int offset = gui->GetDesktop()->GetWinVarOffset(targetVar, &dw);
	if (dw.win || dw.simp) {
	...
	if (offset != -1 && fdw && (fdw->win || fdw->simp)) {
		...
		savefile->Write(&subIndex, sizeof(subIndex));	// savegame version >= 18
	}
	else {
		offset = -1;
		savefile->Write(&offset, sizeof(offset));
...
void idWindow::ReadSaveGameTransition(idTransitionData& trans, idFile* savefile) {
	...
	if (offset != -1) {
		...
		if (session->GetSaveGameVersion() >= 18) {
			savefile->Read(&subIndex, sizeof(subIndex));
		}
		transitionSubs.Append(subIndex);
...
void idWindow::WriteToSaveGame(idFile* savefile) {
	...
	savefile->Write(&textStyle, sizeof(textStyle));	// savegame version >= 18
	savefile->Write(&textSpacing, sizeof(textSpacing));	// savegame version >= 18
...
void idWindow::FixupTransitions() {
	for (int i = 0; i < transitions.Num(); i++) {
		...
		if (transitions[i].data && i < transitionSubs.Num() && transitionSubs[i] != -1) {
			transitions[i].data = new idWinFloatMember(transitions[i].data, transitionSubs[i]);
		}
...
================
*/
bool idWindow::IsVisible() const {
	for (const idWindow* w = this; w; w = w->parent) {
		if (!w->visible) {
			return false;
		}
	}
...
/*
================
idWindow::FindBestChild_r
================
*/
void idWindow::FindBestChild_r(int xd, int yd, int& startX, int& startY, int& bestX, int& bestY, idWindow* current, idWindow*& best) {
	for (int i = children.Num() - 1; i >= 0; i--) {
		...
		if (child->noEvents || !child->scripts[ON_MOUSEENTER] || child == current) {
			child->FindBestChild_r(xd, yd, startX, startY, bestX, bestY, current, best);
			continue;
		}

		const int x = (int)child->actualX;
		const int y = (int)child->actualY;

		if (yd > 0) {
			if (y > startY && (!best || y < bestY)) {
				best = child;
				bestY = y;
			}
		}
		...
		else if (y > startY && (!best || y < bestY)) {
			best = child;
			bestY = y;
		}

		if (xd > 0) {
			if (x > startX && (!best || x < bestX)) {
				best = child;
				bestX = x;
			}
		}
...
/*
================
idWindow::RunControllerScript
================
*/
bool idWindow::RunControllerScript(int script, const sysEvent_t* event, bool* updateVisuals) {
	if (!(flags & WIN_DESKTOP)) {
		return false;
	}

	idWindow* focus = gui->GetDesktop()->focusedChild;
	if (!focus) {
		focus = overChild;
	}
	...
	if (!focus->RunScript(script)) {
		return false;
	}

	// Mirror Q4 behaviour: if focus and hover diverge, move hover to the focused child
	// so controller-only menus still receive the same enter/exit side effects as mouse menus.
	idWindow* newOver = gui->GetDesktop()->focusedChild;
	if (newOver && newOver != overChild) {
		if (overChild) {
			overChild->MouseExit();
			if (overChild->cmd.Length()) {
				AddCommand(overChild->cmd.c_str());
				overChild->cmd = "";
			}
		}
		overChild = newOver;
		dc->SetCursor(newOver->cursor);
		newOver->hover = true;
		newOver->MouseEnter();
		if (newOver->cmd.Length()) {
			AddCommand(newOver->cmd.c_str());
			newOver->cmd = "";
		}
	}
...
/*
================
idWindow::RouteJoystickChange
================
*/
bool idWindow::RouteJoystickChange(int xd, int yd) {
	if (GetCaptureChild()) {
		return GetCaptureChild()->RouteJoystickChange(xd, yd);
	}
	...
	if (overChild && overChild->IsVisible() && !overChild->noEvents && overChild->scripts[ON_MOUSEENTER]) {
		if (xd == 0 && yd == 0) {
			SetFocus(overChild, true);
			dc->SetCursor(overChild->cursor);
			overChild->hover = true;
			overChild->MouseEnter();
			if (overChild->cmd.Length()) {
				AddCommand(overChild->cmd.c_str());
				overChild->cmd = "";
		...
		startX = (int)overChild->actualX;
		startY = (int)overChild->actualY;
	}

	FindBestChild_r(xd, yd, startX, startY, bestX, bestY, overChild, best);

	if (!best) {
		for (int i = 0; i < children.Num(); i++) {
			if (children[i]->RouteJoystickChange(xd, yd)) {
				return true;
			}
		}
		return false;
	...
	SetFocus(best, true);
	dc->SetCursor(best->cursor);
	best->hover = true;

	if (overChild && overChild != best) {
		overChild->MouseExit();
		if (overChild->cmd.Length()) {
			AddCommand(overChild->cmd.c_str());
			overChild->cmd = "";
...
================
*/
void idWindow::SetWrapper(int wrapper) {
	idWinVar* existing = GetWinVarByName("guied_wrapper");
	if (idWinInt* intVar = dynamic_cast<idWinInt*>(existing)) {
		*intVar = wrapper;
		intVar->SetEval(false);
		return;
	}
